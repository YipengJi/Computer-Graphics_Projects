...
public: //------------------------------------------------------ public methods
    ...
    /// constructor
    GLFW_window(const char* _title="", int _width=0, int _height=0, bool visible=true);

    virtual void writeFramebufferImage(const std::string &path) = 0;

    /// destructor
    virtual ~GLFW_window();

    /// main window loop
    int run(const std::string &screenshotPath);

    /// Call this when the content updates.
