...
public:
    ...
    template<typename T>
    void set_uniform(const std::string &name, const T &value, bool optional = false);
...
inline void set_uniform_by_location(int loc, double       val) { glUniform1f       (loc, float(val));                     }
...
template<typename T>
void Shader::set_uniform(const std::string &name, const T &value, bool optional) {
    if (!pid_) return;
    int location = glGetUniformLocation(pid_, name.c_str());
    if (location == -1) {
        if (!optional)
